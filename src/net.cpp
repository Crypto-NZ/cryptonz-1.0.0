...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "CryptoNZ " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"cryptonztools.com", "198.13.37.62"},
    {"cryptonzpool.org", "45.77.51.152"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. CryptoNZ is probably already running."), addrBind.ToString().c_str());
        else
