...
#if defined(NDEBUG)
# error "CryptoNZ cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x4e2b66c36cf7c7d6828a55cc2fdbf27bfd21830a982daaecaa17820b51129e09");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // CryptoNZ: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "CryptoNZ Signed Message:\n";
...
{
    // CryptoNZ: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // CryptoNZ: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 12 * 60 * 60; // CryptoNZ: 6 hours
static const int64 nTargetSpacing = 1 * 60; // CryptoNZ: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // CryptoNZ: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // CryptoNZ: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4e2b66c36cf7c7d6828a55cc2fdbf27bfd21830a982daaecaa17820b51129e09");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NZ Time 22/05/2018, The Coin for all the people of New Zealand";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("650984710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1526940397;
        ...
        assert(block.hashMerkleRoot == uint256("0xfd93481826a60d7a938f2a7d21f0857d5a868f84b807e950ca1f08a9582f9022"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // CryptoNZ: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CryptoNZMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CryptoNZMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CryptoNZMiner(CWallet *pwallet)
{
    printf("CryptoNZMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("CryptoNZ-miner");
    ...
    try { loop {
        ...
        printf("Running CryptoNZMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CryptoNZMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CryptoNZMiner, pwallet));
}
